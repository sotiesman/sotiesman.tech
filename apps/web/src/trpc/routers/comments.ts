...
import { and, asc, comments, count, desc, eq, isNull, rates } from '@sdnsdev/db'
import { CommentNotification } from '@sdnsdev/emails'
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const sendNotification = async (type: 'comment' | 'reply', to: string) => {
        ...
        await resend.emails.send({
          from: 'Nazar from sotiesman.tech <me@sotiesman.tech>',
          to,
          ...
          react: CommentNotification({
            ...
            post: {
              title,
              url: `https://sotiesman.tech/blog/${input.slug}`
            },
            type
          })
        })
